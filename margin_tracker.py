...
# 行の形:
#   B 銘柄名 普通株式 コード5桁 ISIN 売残合計 前週比 買残合計 前週比 \
#   売残一般 前週比 売残制度 前週比 買残一般 前週比 買残制度 前週比
ROW_RE = re.compile(
    r"^[A-Z]\s+(.+?)\s+(?:普通株式|受益証券|投資証券|ＪＤＲ|優先株式|種類株式)"
    r"\s+([0-9A-Z]{5})\s+\S+\s+(.*)$"
)
